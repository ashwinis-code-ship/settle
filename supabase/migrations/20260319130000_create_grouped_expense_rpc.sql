-- Phase 2: RPC to create a grouped expense (expense_group + N child expenses + splits) in one transaction.
-- Runs as INVOKER so RLS applies; caller must be a group member.
...
COMMENT ON FUNCTION public.create_grouped_expense IS 'Creates an expense_group and N child expenses with splits in one transaction. p_lines: JSONB array of { description, amount, split_between: uuid[] }. Requires 2+ lines.';
