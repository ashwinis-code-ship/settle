-- RPC to update a grouped expense: update expense_group row and replace all child expenses/splits.
-- Caller must be a group member. Requires 2+ lines.
...
DECLARE
    ...
    v_count INT;
BEGIN
    -- Get group_id and verify expense_group exists
    ...
    -- Delete existing child expense_splits and expenses (order matters: splits first due to FK)
    ...
    -- Insert new child expenses and splits (same logic as create_grouped_expense)
...
COMMENT ON FUNCTION public.update_grouped_expense IS 'Updates an expense_group and replaces all child expenses/splits. p_lines: JSONB array of { description, amount, split_between: uuid[] }. Requires 2+ lines.';
