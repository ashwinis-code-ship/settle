-- Allow update_grouped_expense to accept 1 line (grouped-by-default: reducing to 1 part updates group in place, no standalone).
-- Avoids creating standalone expenses when user removes all but one part.
...
BEGIN
    ...
    -- Allow 1+ lines (grouped-by-default: single part = update in place, no convert to standalone)
...
COMMENT ON FUNCTION public.update_grouped_expense IS 'Updates an expense_group and replaces all child expenses/splits. p_lines: array of { description, amount, split_between, notes? }. Allows 1+ lines (grouped-by-default).';
