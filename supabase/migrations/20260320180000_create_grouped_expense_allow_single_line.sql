-- Allow create_grouped_expense to accept 1 line (grouped-by-default: single expenses are still expense_groups with one child).
-- No schema change; only relax the line-count check from 2+ to 1+.
...
DECLARE
    ...
    v_count INT;
BEGIN
    -- Require at least one line (grouped-by-default: single expense = 1 line)
...
COMMENT ON FUNCTION public.create_grouped_expense IS 'Creates an expense_group and N child expenses with splits in one transaction. p_lines: JSONB array of { description, amount, split_between: uuid[], notes? }. Allows 1+ lines (grouped-by-default).';
