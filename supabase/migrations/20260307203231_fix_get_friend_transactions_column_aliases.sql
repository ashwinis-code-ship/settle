-- Fix: the final SELECT was projecting CTE columns (ei_id, ei_type, …) back to the
-- declared RETURNS TABLE names (id, type, …) but the implicit mapping was dropping
-- columns in some Postgres versions.  Explicitly alias each output column.
-- No RETURNS TABLE change — same 10-column signature.
