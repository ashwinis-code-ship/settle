-- Fix: PostgreSQL raised "column reference is ambiguous" for `amount` inside the
-- combined CTE because both expense_impacts and settlement_rows expose an unaliased
-- `amount` column and the WHERE clause `WHERE amount != 0` was ambiguous.
-- Qualify the column explicitly in the final filter.
