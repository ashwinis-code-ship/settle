-- Add category_icon and category_color to get_friend_transactions so the friend
-- detail screen can render per-category icons on expense rows.
-- RETURNS TABLE changes (2 new columns) so the old function must be dropped first.
